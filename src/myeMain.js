...
jQuery(function($){
  ...
  // target _blank needs add this to automatically close the window.
  var wsWindow = window.open("", "_self");
  ...
  function ajax_complete_callback(resp) {
    ...
    // can't change the href due to handler needs username and sid while refreshing the page.
    var revurl = window.location.href.split('?')[0];
    var ws_url = revurl.replace('http', 'ws'),
